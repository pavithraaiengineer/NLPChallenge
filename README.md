...
| Naïve Bayes (BoW) | 93.7% |
| Linear SVC (TF-IDF) | 94.5% |
| Random Forest (TF-IDF) | 93.0% |
| MiniLM Embeddings + LR | 95.1% |
| DistilBERT Embeddings + LR | 96.0% |
| DistilBERT Fine-Tuned | 99.61% |
| **Ensemble (Final)** | **99.61%** |

Final model metrics: Accuracy **99.61%** · Precision **0.99** · Recall **0.99** · ROC-AUC **0.9961**
...
We tested stopword removal, stemming (PorterStemmer) and lemmatisation (WordNetLemmatizer) against raw cleaned text. All three preprocessing steps **reduced** model performance.
...
Bag-of-Words and TF-IDF vectorisers (`max_features=5000`) paired with four sklearn classifiers. Best result: **Linear SVC + TF-IDF → 94.5%**.

### 2 — Sentence Embeddings
Static dense vectors from `all-MiniLM-L6-v2` (384-d) and `distilbert-base-uncased` (768-d) used as drop-in replacements for BoW/TF-IDF. No training — weights frozen. Vectors extracted by averaging `last_hidden_state` across the token dimension and wrapped in `csr_matrix` for sklearn compatibility. Best: **DistilBERT Embeddings + LR → 96.0%**.

### 3 — DistilBERT Fine-Tuning
Full fine-tune of `distilbert-base-uncased` on the training set using HuggingFace `Trainer` with AdamW, learning rate `2e-5`, batch size 16, EarlyStopping on eval loss, 2 epochs. Seed 42 outperformed Seed 0. Result: **99.61%**.
...
Result: **99.61% accuracy, 0.9961 ROC-AUC** — most stable configuration across seeds.
...
3. Run `model1.ipynb` for baseline + embedding experiments
4. Run `model2.ipynb` for the full pipeline including DistilBERT and ensemble
...
- Embeddings are processed in batches of 32 to avoid OOM errors
